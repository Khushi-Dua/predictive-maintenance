...
def predict_rul(df):
    ...
    return rul_lstm, rul_xgb, anomaly, None
...
def make_gauge(rul_lstm):
    ...
    rul_clipped = min(max(rul_lstm, 0), 125)
    ax.axvline(rul_clipped, color="white", linewidth=3)
    ax.text(rul_clipped + 1, 0, f"  {rul_lstm:.1f}", color="white",
...
def run_predict(file, use_demo):
    ...
    if use_demo:
        ...
        return (
            "⚠️ Upload a CSV or click Run Demo.",
            "", "", "", None, None,
            gr.update(choices=[])
        )
    ...
    rul_lstm, rul_xgb, anomaly, err = predict_rul(df)
    if err:
        return err, "", "", "", None, None, gr.update(choices=[])

    status  = classify_health(rul_lstm, anomaly)
    gauge   = make_gauge(rul_lstm)

    sensor_options = [f"sensor_{i}" for i in range(1, 22)
                      if f"sensor_{i}" in df.columns]
    default_sensors = sensor_options[:4]
    sensor_fig = make_sensor_plot(df, default_sensors)

    return (
        msg,
        f"{rul_lstm:.1f} cycles",
        f"{rul_xgb:.1f} cycles",
        status,
        gauge,
        sensor_fig,
        gr.update(choices=sensor_options, value=default_sensors)
    )
...
.gr-box { background: #1e2130 !important; border: 1px solid #2d3250 !important; }
...
Forecasts **Remaining Useful Life (RUL)** of industrial equipment from multivariate sensor data.  
Built during internship using NASA CMAPSS dataset as a domain-general proxy for paint shop equipment degradation.
...
---
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            gr.Markdown("""
### Model Results (Official Test Set — 707 units)
| Model | RMSE | MAE |
|---|---|---|
...
**Dataset:** NASA CMAPSS Turbofan Engine Degradation Simulation (FD001–FD004)  
**Models:** Ensemble LSTM (Bagging across 5 bootstrap samples) + XGBoost (Boosting)  
**Anomaly Detection:** Isolation Forest on raw sensor windows  
**Domain Transfer:** The sensor-drift-before-failure pattern applies equally to turbofan engines and paint shop equipment (conveyor motors, curing ovens, pump systems).
    """)
